 ...
 * format:
 *
 * <p>match: test.dispatcher.*.*.* name: dispatcher_events_total labels: action: ${1} outcome:
 * ${2}_out processor: ${0} status: ${1}_${2}
 *
...
public final class MapperConfig {
  ...
  /**
   * Regex used to match incoming metric name. Uses a simplified glob syntax where only '*' are
   ...
   * metric name. Placeholders are in the ${n} format where n is the zero based index of the group
   * to extract from the original metric name. E.g.: match: test.dispatcher.*.*.* name:
   * dispatcher_events_total_${1}
   *
   ...
   * with actual values from the incoming metric name. Placeholders are in the ${n} format where n
   * is the zero based index of the group to extract from the original metric name. E.g.: match:
   * test.dispatcher.*.* name: dispatcher_events_total_${0} labels: label1: ${1}_t
   *
   * <p>A metric "test.dispatcher.sp1.yay" will be converted in a new metric with name
