 ...
 * Process metrics.
 *
 * <p>These metrics are defined in the <a
 * href="https://prometheus.io/docs/instrumenting/writing_clientlibs/#process-metrics">process
 * metrics</a> section of the Prometheus client library documentation, and they are implemented
 * across client libraries in multiple programming languages.
 *
 * <p>Technically, some of them are OS-level metrics and not JVM-level metrics. However, I'm still
 * putting them in the {@code prometheus-metrics-instrumentation-jvm} module, because first it seems
 * overkill to create a separate Maven module just for the {@link ProcessMetrics} class, and seconds
 * some of these metrics are coming from the JVM via JMX anyway.
 *
 * <p>The {@link ProcessMetrics} are registered as part of the {@link JvmMetrics} like this:
 *
 * <pre>{@code
...
public class ProcessMetrics {
   ...
   *
   * <p>A Method object refers to a specific method declared in a specific class. The first
   * invocation might happen with method == SomeConcreteClass.publicLongGetter() and will fail if
   ...
   *
   * <p>There is a built-in assumption that the method will never return null (or, equivalently,
   * that it returns the primitive data type, i.e. {@code long} rather than {@code Long}). If this
