...
public final class HistogramSnapshot extends MetricSnapshot {
  ...
  /**
   * Use this with the first parameter {@code true} to create a snapshot of a Gauge Histogram. The
   * data model for Gauge Histograms is the same as for regular histograms, except that bucket
   * values are semantically gauges and not counters. See <a
   * href="https://openmetrics.io">openmetrics.io</a> for more info on Gauge Histograms.
   */
  ...
  public static final class HistogramDataPointSnapshot extends DistributionDataPointSnapshot {
    ...
    private final int
        nativeSchema; // Number in [-4, 8]. May be CLASSIC_HISTOGRAM for classic histograms.
    ...
    private final NativeHistogramBuckets
        nativeBucketsForPositiveValues; // only used if nativeSchema != CLASSIC_HISTOGRAM
    private final NativeHistogramBuckets
        nativeBucketsForNegativeValues; // only used if nativeSchema != CLASSIC_HISTOGRAM

    /**
     * Constructor for classic histograms (as opposed to native histograms).
     *
     * <p>To create a new {@link HistogramDataPointSnapshot}, you can either call the constructor
     * directly or use the Builder with {@link HistogramSnapshot#builder()}.
     *
     ...
     * directly or use the Builder with {@link HistogramSnapshot#builder()}.
     *
     * @param nativeSchema number in [-4, 8]. See <a
     *     href="https://github.com/prometheus/client_model/blob/7f720d22828060526c55ac83bceff08f43d4cdbc/io/prometheus/client/metrics.proto#L76-L80">Prometheus
     *     client_model metrics.proto</a>.
     * @param nativeZeroCount number of observed zero values (zero is special because there is no
     *     histogram bucket for zero values).
     * @param nativeZeroThreshold observations in [-zeroThreshold, +zeroThreshold] are treated as
     *     zero. This is to avoid creating a large number of buckets if observations fluctuate
     *     around zero.
     * @param nativeBucketsForPositiveValues must not be {@code null}. Use {@link
     *     NativeHistogramBuckets#EMPTY} if empty.
     * @param nativeBucketsForNegativeValues must not be {@code null}. Use {@link
     *     NativeHistogramBuckets#EMPTY} if empty.
    ...
    private static long calculateNativeCount(
        long nativeZeroCount,
        NativeHistogramBuckets nativeBucketsForPositiveValues,
        NativeHistogramBuckets nativeBucketsForNegativeValues) {
     ...
     * Current supported values are -4 &lt;= schema &lt;= 8. See {@link NativeHistogramBuckets} for
     * more info. This will return garbage if {@link #hasNativeHistogramData()} is {@code false}.
     */
    ...
    /**
     * Number of observed zero values. Will return garbage if {@link #hasNativeHistogramData()} is
     * {@code false}.
     */
    ...
    /**
     * All observations in [-nativeZeroThreshold; +nativeZeroThreshold] are treated as zero. This is
     * useful to avoid creation of a large number of buckets if observations fluctuate around zero.
  ...
  public static class Builder extends MetricSnapshot.Builder<Builder> {
    ...
    /**
     * {@code true} indicates that this histogram is a gauge histogram. The data model for gauge
     * histograms is the same as for regular histograms, except that bucket values are semantically
     * gauges and not counters. See <a href="https://openmetrics.io">openmetrics.io</a> for more
